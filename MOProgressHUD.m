...
#define MOMainThreadAssert() NSAssert([NSThread isMainThread], @"MOProgressHUD needs to be accessed on the main thread.");

CGFloat const MOProgressMaxOffset = 1000000.f;

static const CGFloat MODefaultPadding = 4.f;
static const CGFloat MODefaultLabelFontSize = 16.f;
static const CGFloat MODefaultDetailsLabelFontSize = 12.f;


@interface MOProgressHUD ()
...
@interface MOProgressHUDRoundedButton : UIButton
@end
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    MOProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    MOProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (MOProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            MOProgressHUD *hud = (MOProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = MOProgressHUDAnimationFade;
    _mode = MOProgressHUDModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(MOProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == MOProgressHUDAnimationZoom) {
        type = animatingIn ? MOProgressHUDAnimationZoomIn : MOProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == MOProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == MOProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == MOProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == MOProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    MOProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<MOProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    MOBackgroundView *backgroundView = [[MOBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = MOProgressHUDBackgroundStyleSolidColor;
    ...
    MOBackgroundView *bezelView = [MOBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:MODefaultLabelFontSize];
    label.opaque = NO;
    ...
    UIButton *button = [MOProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:MODefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[MORoundProgressView class]];

    MOProgressHUDMode mode = self.mode;
    if (mode == MOProgressHUDModeIndeterminate) {
    ...
    else if (mode == MOProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[MOBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == MOProgressHUDModeDeterminate || mode == MOProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[MORoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == MOProgressHUDModeAnnularDeterminate) {
            [(MORoundProgressView *)indicator setAnnular:YES];
        }
    }
    else if (mode == MOProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == MOProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[MOProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[MOProgressHUD class]]];
#endif

        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[MORoundProgressView class]]) {
        MORoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MORoundProgressView appearanceWhenContainedIn:[MOProgressHUD class], nil];
#else
        appearance = [MORoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[MOProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((MORoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((MORoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[MOBarProgressView class]]) {
        MOBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MOBarProgressView appearanceWhenContainedIn:[MOProgressHUD class], nil];
#else
        appearance = [MOBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[MOProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((MOBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((MOBarProgressView *)indicator).lineColor = color;
        }
    } else {
...
- (void)updateBezelMotionEffects {
    MOBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? MODefaultPadding : 0.f;
...
- (void)setMode:(MOProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == MOProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation MORoundProgressView
...
@implementation MOBarProgressView
...
@implementation MOBackgroundView

#pragma mark - Lifecycle

- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        _style = MOProgressHUDBackgroundStyleBlur;
...
- (void)setStyle:(MOProgressHUDBackgroundStyle)style {
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
    }
...
- (void)updateForBackgroundStyle {
    ...
    MOProgressHUDBackgroundStyle style = self.style;
    if (style == MOProgressHUDBackgroundStyleBlur) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == MOProgressHUDBackgroundStyleBlur) {
        self.backgroundColor = self.color;
    } else {
        self.backgroundColor = self.color;
    }
...
@implementation MOProgressHUDRoundedButton
